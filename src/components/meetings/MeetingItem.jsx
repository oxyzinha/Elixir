...
const MeetingItem = ({ meeting }) => {
  ...
  const handleActionClick = () => {
    if (meeting.status === 'Ativa' || meeting.status === 'Próxima') {
      if (meeting.id) {
        navigate(`/meeting/${meeting.id}`); // Navega para a sala de Consulta
      } else {
        toast({
          title: "Erro de navegação",
          description: "ID da Consulta não encontrado para entrar.",
          variant: "destructive",
          duration: 3000,
        });
      }
      ...
      if (meeting.id) {
        ...
        toast({
          title: "Erro de navegação",
          description: "ID da Consulta não encontrado para ver detalhes.",
          variant: "destructive",
          duration: 3000,
        });
      }
