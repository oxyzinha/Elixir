...
import { 
  MessageSquare, FileText, CalendarClock, Code2, Users, ArrowRight, Video, 
  Pill, CalendarCheck, Info, Clock 
} from 'lucide-react'; 
...
// Mapeamento de ícones (MUITO IMPORTANTE que esteja alinhado com ActivityNotificationContext)
const iconMap = {
  ...
  'meeting-full': { icon: CalendarClock, iconColorClass: 'text-yellow-600', bgColorClass: 'bg-yellow-50' }, // Consistência
  ...
  medicacao: { icon: Pill, iconColorClass: 'text-pink-600', bgColorClass: 'bg-pink-50' }, // NOVO TIPO
  documentos: { icon: FileText, iconColorClass: 'text-cyan-600', bgColorClass: 'bg-cyan-50' }, // RECEITA
};
...
const ActivityItem = ({ item }) => {
  ...
  const handleLinkClick = (e) => {
    e.preventDefault(); // Evita que o link HTML padrão seja seguido
  ...
  const formatDate = (isoString) => {
    if (!isoString) return '';
    try {
      // Ajuste para lidar com "Há X minutos/horas"
      if (isoString.includes("Há ")) return isoString; 
  ...
  const formatTime = (isoString) => {
    ...
    try {
      // Ajuste para lidar com "Há X minutos/horas"
      if (isoString.includes("Há ")) return ''; 
    ...
    >
      <div className="flex items-center gap-4 w-full">
        <div className="flex-shrink-0 flex flex-col items-center gap-2">
          ...
          {/* Renderizar avatar fallback apenas se avatarFallback existir */}
          {item.avatarFallback && (
        ...
        {item.link && (
          <a
            href={item.link}
            onClick={(e) => {
              e.stopPropagation(); // Previne que o click na âncora duplique o onClick da div pai
              handleLinkClick(e);
            }}
            className={`
